...
def get_condition_tokens(vqgan, image_path, seed_length):
    image = Image.open(image_path).convert("RGB").resize((256, 256))
    image_tensor = torch.tensor(np.array(image)).permute(2, 0, 1).float() / 127.5 - 1
    image_tensor = image_tensor.unsqueeze(0)  # Add batch dim
    with torch.no_grad():
...
def main():
    ...
    sampled = sample_with_masking(combined_tokens.unsqueeze(0),encodedTokens.unsqueeze(0),transformer, steps=256 - combined_tokens.size(0), temperature=0.5, top_k = 100, top_p = 1.0)
